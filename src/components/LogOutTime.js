...
const LogOutTime = () => {
    ...
    const getLoginTime = () => {
        Axios.get("http://172.16.224.250:5000/logout/getallAttendance", {
            headers: {
    ...
    const deleteData = (value) => {
        ...
        Axios.post("http://172.16.224.250:5000/logout/deleteData", { data: newData }, {
            headers: {
                 contentType: "application/json",
                 Authorization:"Bearer "+localStorage.getItem("token")
            }
...
useEffect(() => {
    let empIds = users.map(item => item._id)
    setvalue(empIds)
}, [users])

    const hadledata = () => {
      
        Axios.get(`http://172.16.224.250:5000/logout/getByEmpId/${newValue}`, {
            headers: {
                contentType: "application/json",
                Authorization:"Bearer "+localStorage.getItem("token")
            }
    ...
    return (
        <div>
            { loading ?
                ...
                <div className="container">
                    <h1 className="pt-2">Employee LogOut Time</h1>
                    <Autocomplete
                    options={users}
      ...
      columns={[
        ...
        { title: 'Logout Date', field: 'logoutDate', type: 'numeric' },
        ...
        {title:"Total Working hrs", field:"totalWorkingHrs"}
